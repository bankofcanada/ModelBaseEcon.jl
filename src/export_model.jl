...
# ----------------------------------------------------------------------
# export_model - write a Model out as a runnable .jl file
#
# The emitted file defines `build_<name>()` which reconstructs the model
# via the DSL (`@parameters`, `@variables`, `@logvariables`,
# `@shocks`, `@exogenous`, `@autoexogenize`, `@equations`, `@initialize`).
# Round-trip target: per-equation `expr_hash` equal to the original, and
# the flat-parameter vector byte-identical.
#
# `@steadystate` user constraints and array-param emission with
# tag/doc metadata are emitted to the extent they exist on the model.
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# Emit `@shocks` and `@exogenous`. Both are stored in `defs.shocks`; the
...
# Emit `@steadystate m begin ... end` so the round-trip
...
Caller is responsible for `using ModelBaseEcon` in the consumer file
before `include`ing the emitted source - the emitted source assumes the
DSL macros are in scope.

Limitations:
- `@steadystate` user constraints are emitted when present on the model;
...
function _emit_file(io::IO, def::IR.ModelDef, name::Symbol, build_fn::Symbol)
    println(io, "# Generated by ModelBaseEcon.export_model - do not edit by hand.")
